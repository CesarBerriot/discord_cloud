		...
		if(ImGui::Button("Upload"))
			...
			(	[]
				{	variables::mutex.lock();
					...
					logger::message("Archiving Directory...");
					archiver::splits_t splits = archiver::archive(variables::directory);
					logger::message(std::format("Processing {} Part(s)...", splits.size()));
					request_handler::run
					(	std::format("channels/{}/messages", variables::selected_channel),
						{},
						{ { { "content", std::to_string(splits.size()) } } }
					);
					...
					{	std::string name = std::to_string(i + 1);
						...
						(	std::format
							(	"Uploading Part {} ({}%)...",
								name,
								::helpers::compute_percentage(splits.size(), i + 1)
							)
						);
						request_handler::run
						(	std::format("channels/{}/messages", variables::selected_channel),
