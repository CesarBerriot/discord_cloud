	...
	auto request =
		...
		{	std::vector<std::function<void()>> cleanup_procedures;
			...
			{	curlpp::Forms form_parts;
				...
				std::filesystem::path temporary_directory = std::filesystem::temp_directory_path();
				for(int i = 0; ; ++i)
				{	std::filesystem::path new_temporary_directory = temporary_directory / std::to_string(i);
					if(!std::filesystem::exists(new_temporary_directory))
					{	std::filesystem::create_directory(new_temporary_directory);
						temporary_directory = new_temporary_directory;
						break;
					}
				}
